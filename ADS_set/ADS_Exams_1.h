...
public:
  ...
  const_iterator x() const{
    ...
    return const_iterator{bucket_ptr,*bucket_ptr,&table[table_size],Mode::special,current_size - 2*(current_size/4) ,current_size/4};
...
private:
  ...
  Element ** end_bucket;
  Mode mode;
  size_type number_of_elem;
  size_type number_skips;
  size_type count = 0;
  void skip()
  {
...
public:
  ...
  explicit Iterator(Element ** bucket = nullptr,Element * node = nullptr,Element ** end_bucket = nullptr,Mode mode = Mode::normal,size_type number_of_elem = 0,size_type number_skips = 0) : bucket{bucket},node{node},end_bucket{end_bucket},mode{mode},number_of_elem{number_of_elem},number_skips{number_skips} 
  { 
    if(mode == Mode::normal){
    if(bucket){
      if(node){
  ...
  skip();
  for(size_type i = 0; i < number_skips;i++){
    ++(*this);
  }
  count = 0;
    }
  ...
  {
    ++count;
    if(mode == Mode::normal || number_skips == 0 ){
        if(node)
        {
        node = node->next;
        }
