...
public:
  ...
  void swap(ADS_set &other);
    ...
    return const_iterator{bucket_ptr,*bucket_ptr,&table[table_size],current_size/3,current_size,Mode::special};
  }
...
public:
  ...
  explicit Iterator(Element ** bucket = nullptr,Element * node = nullptr,Element ** end_bucket = nullptr,size_type first_elements = 0,size_type number_elements = 0,Mode mode = Mode::normal) : bucket{bucket},node{node},end_bucket{end_bucket},first_elements{first_elements},number_elements{number_elements},mode{mode} 
  { 
    if(bucket){
      if(node){
        this->node = node;
  ...
  skip();
  if(first_elements == 0 && mode == Mode::special){
    this->bucket = this->end_bucket;
    this->node = nullptr;
  }
